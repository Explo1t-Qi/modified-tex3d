...
相同 seed、State 0 训练帧和零初始化谱系数上的单步标定结果为：

| 单一目标 | 权重 | 谱系数梯度范数 |
|---|---:|---:|
| action-only | `alpha_action=1` | `66.26807` |
| SigLIP-only | `alpha_feature=1` | `1.602139` |

两个系数更新方向的余弦相似度为 `+0.3011`；再由 action-only、feature-only
范数推算原 `alpha_feature=10` 混合梯度的方向余弦为 `+0.3073`，二者接近，
也与实测混合梯度范数 `72.80559` 一致。这说明两个目标在首帧谱空间中轻度
协同，并不存在需要梯度投影处理的直接冲突。

要让两个目标的初始加权梯度等强，需要
`alpha_feature / alpha_action = 66.26807 / 1.602139 = 41.36`。第一轮正式
Shared-SigLIP pilot 采用便于复现的近似值：
...
此时 feature/action 加权梯度范数比约为 `0.967`。由于谱更新按曲面变化做全局
归一化，`0.1/4.0` 与 `1/40` 的首步方向等价；保留较小总 loss 数值仅便于和
