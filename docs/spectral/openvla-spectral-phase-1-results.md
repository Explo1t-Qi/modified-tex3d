...
当前后续路线选择“优先快速验证机制”：保持 OpenVLA source-only 训练，OFT
梯度只用于诊断，不进入 loss 或选基。K=256 renderer VJP 已实现，能够把主视角
与腕部像素梯度映射到同一 `[256,3]` 系数空间；GPU 运行命令和结果判读继续维护
在上述像素梯度审计文档中。首轮目标不是完整消融，而是在不降低当前 OpenVLA
30%攻击成功率的前提下，让 OFT 从0/10出现至少2/10失败的非零迁移信号。

多实例修正后的 K=256 VJP 已完成：主视角真实扰动覆盖率为94.42%，跨模型
Feature/Action 系数 cosine 为 `0.662 / 0.228`。当前 Source 加权目标对 OFT
主视角Action尚有 `0.281`，但对双视角Action仅 `0.133`；OFT腕部Action梯度为
主视角的3.31倍，而两视角Action方向 cosine 仅0.008。由此将下一条最小机制
