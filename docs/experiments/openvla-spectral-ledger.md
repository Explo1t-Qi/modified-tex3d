...
| 6j（当前唯一下一门槛） | Radial-vs-Matched-Box Counterfactual正式audit | commit `4752b18`的state 0 smoke及WSL独立复核已通过；下一步在另一全新目录完成states 0--9正式60-response bundle | 工程上要求parent raw replay、2个matched step与60条response全部有效；科学上只分别报告每endpoint的`D=L_radial-L_matched`与`I_M=L_baseline-L_matched`及逐state符号计数，不设隐藏pass阈值 | 不重算梯度、不训练、不rollout、不调Support/K/lambda/Feature/wrist/OFT；正式结果前不解释projection机制 |
