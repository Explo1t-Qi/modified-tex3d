...
| 6j（当前唯一下一门槛） | Radial-vs-Matched-Box Counterfactual | commit `8cfa791`已完成artifact/evaluator、state 0 smoke与正式runner；先在新目录完成双endpoint×state 0×三臂6-response smoke并严格replay parent，再在另一新目录完成正式60-response bundle | 工程上要求parent raw replay、2个matched step与60条response全部有效；科学上只分别报告每endpoint的`D=L_radial-L_matched`与`I_M=L_baseline-L_matched`及逐state符号计数，不设隐藏pass阈值 | 不重算梯度、不训练、不rollout、不调Support/K/lambda/Feature/wrist/OFT；GPU结果出来前不解释projection机制 |
