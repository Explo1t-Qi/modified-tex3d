...
| 6i（待运行） | Terminal Endpoint Action-Gradient/Response Audit | commit `953c21c`已实现GPU runner；先通过服务器无GPU定向测试，再在同一干净commit采集双终态20 gradient、2 step、60 response并rsync回WSL | CPU独立复核`audit_valid=true`、20/60/2 inventory和全部SHA/processor/state/step派生量完整；只报告冻结的retention与Dense-vs-Support描述量 | 工程无效则只修数据链路并在新目录重跑；工程有效后按预注册边界讨论，不把局部单步外推为训练轨迹或rollout因果 |
