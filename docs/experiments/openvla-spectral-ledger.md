...
| 2026-08-16 | Gate 6j Radial-vs-Matched-Box合同与实现 | commits `057a897`/`8cfa791`；只读Gate 6i双终态、聚合Action gradient与Support radial step，构造coordinatewise box point后按parent radial actual Surface-L∞缩放；不重新backward、训练或rollout | child bundle自包含parent Gate 6i、Dense Seed与Production Support；正式固定2 endpoint×10 states×3 arms=60 responses，并对40条baseline/radial执行raw严格replay；逐endpoint报告`D=L_radial-L_matched`和`I_M=L_baseline-L_matched`，不设科学布尔Gate；state 0 smoke固定6 responses且标记非正式 | artifact/evaluator、GPU smoke/formal runner已完成；Gate 6i/6j定向24项通过，本地全量仍因缺`nvdiffrast`/完整LIBERO在10个既有文件collection停止；尚无GPU或科学结果，下一步先smoke再formal | [当前状态](../status/openvla-spectral-current.md) |
