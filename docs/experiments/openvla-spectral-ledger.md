...
| 6（进行中） | 谱自然性约束 + Fixed Vertex Support源候选 | 全部基础Gate与Dense Seed Gradient Audit通过 | 先冻结并验证Seed Score、平滑与Support Construction契约；候选held-out states 10–19至少3/10失败 | 先归因参数化/目标/部署输入，不做无依据网格扫描 |
