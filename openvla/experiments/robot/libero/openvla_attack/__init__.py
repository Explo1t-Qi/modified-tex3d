...
图像合成、renderer、运行时资源事务、实验产物与单 episode 评估逻辑。这里不在
包初始化时导入 ``evaluation``、``frame_collection``、``optimization``、
``runtime_assets`` 和 ``artifacts``，避免只使用纯计算 module 的调用方被迫
加载 LIBERO/Robosuite 或图像/视频 I/O。
"""
